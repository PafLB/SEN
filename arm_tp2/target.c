...
#ifdef __WITH_GPIO_DEV
    	if (button0()) {
			sys_sem_v(dev_btn0.sem_read);
    	
    	} else if (button1()) {
			sys_sem_v(dev_btn1.sem_read);
    	
    	} else if (button2()) {
			sys_sem_v(dev_btn2.sem_read);
    	
    	} else if (button3()) {
			sys_sem_v(dev_btn3.sem_read);
   	
    	} else if (button4()) {
			sys_sem_v(dev_btn4.sem_read);
...
Device dev_console={
    ...
    .sem_read=dev_read_console,
    .sem_write=dev_write_console,
    .open=dev_open_console,
    .close=dev_close_console,
    .read=NULL,
    .write=NULL,
    .ioctl=NULL
};
